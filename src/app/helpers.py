...
def get_metadata_by_priority(
    media_type,
    source_ids: dict,
    title,
    season_number=None,
):
    def get_source_priority(media_type):
        """
        Return sources in descending priority order.
    ...
    """
    for source in get_source_priority(media_type):
        ...
        try:
            ...
            pass
            # import sys
            # logger.error(f'Error on line {type(e).__name__} - {sys.exc_info()[-1].tb_lineno} - {e}')
